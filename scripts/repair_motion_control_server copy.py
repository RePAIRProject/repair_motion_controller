...
class RepairMotionControlServer:
    def __init__(self, name):
        ...
        #self._robot = rospy.get_param(rospy.search_param('robot'), 'sim')

        rospy.logwarn(f"ROBOT: {self._is_vis_enable}   |    {self._robot}")
        xy = rospy.get_param('/use_sim_time')
        print(xy)
    ...
    def action_executor(self, goal: RepairMoveToGoal):
        ...
        try: 
            ...
            self.terminate_planning()
            print("Problem with planning yikes sad 1")
        ...
        if traj_msg is None:
            ...
            self.terminate_planning()
            print("Problem with planning yikes sad 2")
            self._planner.planner_robot.setConfig(start_config)
            return
    ...
    def move_robot_to_goal(self, joint_trajectory):
        ...
        print(joint_trajectory.points[0].positions)
        print(joint_trajectory.points[-1].positions)
        ...
        if(True):
            print("MY RESULT FOR WAITING SUPER DUPER: ",result)
        
        # if(result.error_code == 0):
        #     print("Setting joint trajectory")
        #     print("current config: ",self._planner.planner_robot.getConfig())
        #     last_point = joint_trajectory.points[-1]
        ...
        #     print(dict(zip(joint_names, positions)))
            
        #     full_config = self._planner.planner_robot.getConfig()
        #     joint_names = joint_trajectory.joint_names
        #     positions = last_point.positions
        #     joint_map = dict(zip(joint_names, positions))
            
        #     for i in range(len(full_config)):
        #         link_name = self._planner.planner_robot.link(i).getName()
        #         if link_name in joint_map:
        #             full_config[i] = joint_map[link_name]

        #     print(full_config)
            
        #     #print("joint_trajectory: ",joint_trajectory)
        #     self._planner.planner_robot.setConfig(full_config)
